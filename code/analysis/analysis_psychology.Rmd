...
plot_data_psych <- data_processed_trimmed |>
  mutate(n_total = n()) |>
  group_by(alpha) |>
...
ggsave("plots/p_kernel_psychology.png",
       plot = p_kernel_psych,
       device = png,
       width = 6,
       height = 4.5,
...
write_csv(data_inflation_kernel, "plots/residuals_psychology.csv")
...
data_caliper <- data_processed_trimmed %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
  summarise(count = sum(!is.na(alpha)))
...
plot_data_psych_beta <- data_processed_trimmed |>
  mutate(n_total = n()) |>
  group_by(alpha) |>
  summarise(count = sum(!is.na(alpha))) |>
  mutate(type = case_when(alpha %in% c(.70, .80, .90) ~ "Common reliability threshold",
...
plot_data <- data_processed_trimmed |>
  mutate(n_total = n()) |>
  group_by(alpha) |>
  summarise(count = sum(!is.na(alpha))) |>
