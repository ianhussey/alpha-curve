...
data_frequency_temp <- data_processed %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
...
total_n <- nrow(data_processed)/100

plot_data_io <- data_processed |>
  mutate(n_total = n()) |>
  group_by(alpha) |>
...
write_csv(data_inflation_kernel, "plots/residuals_io.csv")
