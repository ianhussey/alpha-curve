...
data_inflation_kernel <- data_frequency %>%
  ...
  mutate(predicted = predicted/100,
         ...
         group_7_cutoffs = as.factor(case_when(alpha %in% c(.60, .65, .70, .75, .80, .85, .90) ~ "cutoff",
                                               TRUE ~ "non cutoff")),
...
.60, .65, .70, .75, .80, .85, .90 against the rest (NB this test is more exploratory)

```{r}

independence_test(residual ~ group_7_cutoffs,
                  data = data_inflation_kernel,
