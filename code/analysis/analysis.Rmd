...
data_inflation_beta <- data_frequency %>%
  ...
  mutate(predicted = predicted/100,
         residual = residual/100,
         inflation = janitor::round_half_up((observed/predicted - 1)*100, digits = 0)) %>%
  mutate(group = as.factor(case_when(alpha %in% c(.70, .80, .90) ~ "cutoff",
                                     TRUE ~ "non cutoff")))
...
- There were `r data_inflation_kernel$inflation[data_inflation_kernel$alpha == 0.69]`% fewer observations of Cronbach's $a$ = .69 and `r data_inflation_kernel$inflation[data_inflation_kernel$alpha == 0.70]`% more observations of Cronbach's $a$ = .70 than predicted.
- There were `r data_inflation_kernel$inflation[data_inflation_kernel$alpha == 0.79]`% fewer observations of Cronbach's $a$ = .79 and `r data_inflation_kernel$inflation[data_inflation_kernel$alpha == 0.80]`% more observations of Cronbach's $a$ = .80 than predicted.
- There were `r data_inflation_kernel$inflation[data_inflation_kernel$alpha == 0.89]`% more observations of Cronbach's $a$ = .89 and `r data_inflation_kernel$inflation[data_inflation_kernel$alpha == 0.90]`% more observations of Cronbach's $a$ = .90 than predicted.
