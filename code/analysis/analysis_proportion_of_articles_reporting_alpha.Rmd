...
# Combine journal and alpha

```{r}

data_journals_alphas <- 
  full_join(data_journals, data_alphas, by = "doi") |>
  # if alpha is missing, there isn't one, so correct to false
  mutate(alpha = ifelse(is.na(alpha), FALSE, alpha))
...
data_journals_alphas_fields <- data_journals_alphas |>
  mutate(field = case_when(
    journal %in% j_behaviorism ~ "behaviorism",
    journal %in% j_clincial ~ "clincial",
