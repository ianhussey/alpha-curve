...
n_estimates_before_exclusions <- data_processed |>
  nrow() 

cat("n alpha estimates before exclusions =", n_estimates_before_exclusions, "\n")
...
n_doi_by_test_type <- data_psyctests |>
  ...
  mutate(test_type = case_when(test_type == "Original" ~ "Original",
                               test_type %in% c("Revision", "Translation") ~ "Revision or translation",
                               TRUE ~ test_type)) |>
  count(test_type, name = "n_dois")

n_estimates_after_exclusions_by_test_type <- data_processed_trimmed |>
  mutate(test_type = case_when(test_type == "Original" ~ "Original",
                               test_type %in% c("Revision", "Translation") ~ "Revision or translation",
                               TRUE ~ test_type)) |>
...
n_by_test_type <- 
  full_join(n_doi_by_test_type,
            ...
            by = "test_type") |>
  mutate(proportion_dois_giving_one_or_more_estimates = janitor::round_half_up(n_dois / n_estimates * 100, digits = 1)) |>
  arrange(desc(n_dois))
...
plot_data_psyctests <- data_processed_trimmed |>
  mutate(n_total = n()) |>
  group_by(alpha) |>
...
write_csv(data_inflation_kernel, "plots/residuals_psyctests.csv")
...
data_processed_trimmed_temp <- data_processed_trimmed |>
  filter(test_type == "Original")
  
nrow(data_processed_trimmed_temp)
...
data_frequency_temp <- data_processed_trimmed_temp %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
  summarise(freq = sum(!is.na(alpha))/first(n_total)) %>%
  # convert alpha to character to facilitate a join later
...
# kernel density estimation
data_density <- 
  density(data_processed_trimmed_temp %>%
            pull(alpha),
          n      = length(seq(0.01, 0.99, 0.01)),
          from   = 0.01,
          to     = 0.99, 
...
total_n <- nrow(data_processed_trimmed_temp)/100

plot_data_psyctests <- data_processed_trimmed_temp |>
  mutate(n_total = n()) |>
  group_by(alpha) |>
  summarise(count = sum(!is.na(alpha))) |>
  mutate(type = case_when(alpha %in% c(.70, .80, .90) ~ "Common reliability threshold",
...
data_caliper <- data_processed_trimmed_temp %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
  summarise(count = sum(!is.na(alpha)))
