...
Values are hard-coded and may need updating if results changed!

```{r fig.height=4.5, fig.width=5.0}

dat <- 
  tibble(Dataset = c("Psychology", "I/O", "PsycTests"),
         excess_70 = c(14, 14, 12),
         excess_80 = c(3, 2, 5),
         excess_90 = c(3, 1, 0)) |>
  pivot_longer(cols = starts_with("excess_"),
               names_to = "Threshold",
               values_to = "Excess") |>
  mutate(Excess = Excess + 0.05) |> # offset so that 0% is still visible in the plot
  mutate(Dataset = fct_relevel(Dataset, "Psychology", "I/O", "PsycTests"))

custom_labels <- c("excess_70" = "α = .70", 
                   "excess_80" = "α = .80", 
                   "excess_90" = "α = .90")

#library(ggsci)
#pal_simpsons("springfield")(12)
#show_col(pal_simpsons("springfield")(15))
...
plot_combined_excesses <- 
  ...
  scale_y_continuous(labels = percent_format(scale = 1), breaks = seq(0, 100, by = 2.5), name = "Excess (Residual)") + # y-axis in percent with increments of 2.5%
