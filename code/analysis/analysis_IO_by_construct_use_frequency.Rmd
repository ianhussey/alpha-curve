...
construct_counts <- data_processed |>
  count(Var1)

data_processed_constructs_used_once <- data_processed |>
  semi_join(construct_counts |>
              filter(n == 1), 
            by = "Var1")

data_processed_constructs_used_many_times <- data_processed |>
  semi_join(construct_counts |>
              filter(n > 1 & n < 100), 
            by = "Var1")

data_processed_constructs_used_100 <- data_processed |>
  semi_join(construct_counts |>
              filter(n >= 100), 
            by = "Var1")
...
nrow(data_processed_constructs_used_once)

data_frequency_temp <- data_processed_constructs_used_once %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
...
data_density <- 
  density(data_processed_constructs_used_once %>%
            pull(alpha),
...
total_n <- nrow(data_processed_constructs_used_once)/100
...
nrow(data_processed_constructs_used_many_times)

data_frequency_temp <- data_processed_constructs_used_many_times %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
  summarise(freq = sum(!is.na(alpha))/first(n_total)) %>%
  # convert alpha to character to facilitate a join later
...
# kernel density estimation
data_density <- 
  density(data_processed_constructs_used_many_times %>%
            pull(alpha),
          n      = length(seq(0.01, 0.99, 0.01)),
          from   = 0.01,
          to     = 0.99, 
...
total_n <- nrow(data_processed_constructs_used_many_times)/100
...
nrow(data_processed_constructs_used_100)

data_frequency_temp <- data_processed_constructs_used_100 %>% 
  mutate(n_total = n()) %>% 
  group_by(alpha) %>% 
  summarise(freq = sum(!is.na(alpha))/first(n_total)) %>%
  # convert alpha to character to facilitate a join later
...
# kernel density estimation
data_density <- 
  density(data_processed_constructs_used_100 %>%
            pull(alpha),
          n      = length(seq(0.01, 0.99, 0.01)),
          from   = 0.01,
          to     = 0.99, 
...
total_n <- nrow(data_processed_constructs_used_100)/100
