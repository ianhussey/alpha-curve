...
#                                recursive  = TRUE, 
#                                full.names = TRUE)) |>
#   mutate(doi  = str_remove(filename, "/fulltext.text"), 
...
data_extracted_cronbach_alpha <- data_fulltexts |>
  ...
  mutate(locations = list(
    as.data.frame(
      str_locate_all(string = text, pattern = "(C|c)ronbach('|’)?s? (alpha|a|α)")
    ) |>
  ...
  mutate(pre  = str_sub(text,
                        start = location_start - 50,
                        end   = location_start - 1),
         post = str_sub(text,
                        start = location_end + 1,
                        end   = location_end + 50),
         all = paste(pre, "Cronbach's alpha", post, sep = ""),
         regex_label = "Cronbach's alpha",
         regex_inclusion = "(C|c)ronbach('|’)?s? (alpha|a|α)",
         regex_exclusion = NA) |>
  select(doi, 
...
## Internal consistency ... alpha (without Cronbach)

```{r}

data_extracted_internal_consistency_and_alpha <- data_fulltexts |>
  # first find reference to internal consistency
  rowwise() |>
  mutate(locations = list(
    as.data.frame(
      str_locate_all(string = text, pattern = "internal consistenc")
    ) |>
      rownames_to_column(var = "exemplar")
  )) |>
  ungroup() |>
  ...
  mutate(locations = list(
    as.data.frame(
      str_locate(string = post_ic, pattern = "(alpha|a|α)")
    ) 
  )) |>
  ungroup() |>
  unnest(locations) |>
  ...
  mutate(pre  = str_sub(text,
                        start = location_end + location_start_alpha - 50,
                        end   = location_end + location_start_alpha - 1),
         post = str_sub(text,
                        start = location_end + location_end_alpha + 1,
                        end   = location_end + location_end_alpha + 50),
         all = paste(pre, "α", post, sep = ""),
         regex_label = "internal consistency ... alpha",
         regex_inclusion = "c('internal consistenc', '(alpha|a|α)')",
         regex_exclusion = "(C|c)ronbach('|’)?s?") |>
  # then exclude strings with "cronbach" as this is caught by the previous extraction
  filter(!str_detect(all, "(C|c)ronbach('|’)?s?")) |>
  select(doi, 
         regex_label, regex_inclusion, regex_exclusion, 
         exemplar, location_start, location_end, 
         pre, post, all)
...
```{r}
# 
# data_extracted_internal_consistency <- data_fulltexts_subset |>
#   # first find reference to internal consistency
#   rowwise() |>
#   mutate(locations = list(
#     as.data.frame(
...
#                         start = location_end + 1,
#                         end   = location_end + 50),
#          all = paste(pre, "internal consistenc", post, sep = ""),
#          regex_label = "internal consistency",
#          regex_inclusion = "internal consistenc",
#          regex_exclusion = "c('(C|c)ronbach('|’)?s?', '(alpha|a|α)')") |>
#   # then exclude strings with "cronbach" as this is caught by the previous extraction
#   filter(!str_detect(all, "(C|c)ronbach('|’)?s?") |
#            !str_detect(all, "(alpha|a|α)")) |>
#   select(doi, 
#          regex_label, regex_inclusion, regex_exclusion, 
#          exemplar, location_start, location_end, 
#          pre, post, all)
...
data_extracted <- 
  bind_rows(data_extracted_cronbach_alpha, 
            data_extracted_internal_consistency_and_alpha)
