...
data_extracted_cronbach_alpha <- data_fulltexts |>
  ...
  mutate(locations = list(
    as.data.frame(
      str_locate_all(string = text, pattern = "(C|c)ronbach('|’)?s? (Alpha|alpha|a|α)")
    ) |>
  ...
  mutate(pre  = str_sub(text,
         ...
         regex_inclusion = "(C|c)ronbach('|’)?s? (Alpha|alpha|a|α)",
         regex_exclusion = NA) |>
  select(doi, 
...
data_extracted_internal_consistency_and_alpha <- data_fulltexts |>
  ...
  mutate(locations = list(
    as.data.frame(
      str_locate(string = post_ic, pattern = "(Alpha|alpha|α)")
    ) 
  )) |>
  ungroup() |>
  unnest(locations) |>
  ...
  mutate(pre  = str_sub(text,
         ...
         regex_inclusion = "c('(I|i)nternal ((C|c)onsistenc|(R|r)eliabilit)', '(Alpha|alpha|α)')",
...
data_extracted_internal_consistency <- data_fulltexts |>
  ...
  mutate(pre  = str_sub(text,
         ...
         regex_exclusion = "c('(C|c)ronbach('|’)?s?', '(Alpha|alpha|α)')") |>
  # then exclude strings with "cronbach" as this is caught by the previous extraction
  filter(!str_detect(all, "(C|c)ronbach('|’)?s?") &
           !str_detect(all, "(Alpha|alpha|α)")) |>
  select(doi,
         regex_label, regex_inclusion, regex_exclusion,
         exemplar, location_start, location_end,
         pre, post, all)
