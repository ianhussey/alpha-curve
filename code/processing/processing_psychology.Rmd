...
data_estimates <- data_extracted |>
  mutate(
    ...
    # exclusion variables
    exclude_comparison_present = str_detect(substring_between_cronbach_and_estimate, "([<>≥≤±≈]|over|larger|exceed|above|upper|greater|higher|more|lower|below|smaller|less|at least|between|the order of|in the region of|approximately|minimum|in excess of|under|within|around|surpass)"),
    ...
    exclude_comparison_present_after_estimate = str_detect(post, "(and above|or above|and higher|or higher|and more|or more|and greater|or greater|and below|or below|and less|or less|and lower|or lower)"),
    exclude_s = str_starts(exclude_comparison_present_after_estimate, "s"), # eg "in the .70s"
    exclude_cutoff_present = str_detect(all, "cut(\\s|-)off|cutoff|criteria|is considered|is generally considered|threshold|the minimum considered acceptable|general rule of thumb"),
    exclude_range_present = str_detect(substring_between_cronbach_and_estimate, "(range|ranging|between|from)"),
    exclude_range_present_2 = str_detect(all, "(.70s|.80s|.90s|mid to high|exceeded α =)"),
    ...
    exclude_odd_string_present_2 = str_detect(all, "test-retest reliaiblity and alpha"),
    exclude_kappa = str_detect(all, "(kappa|κ)"),
    exclude_splithalf = str_detect(all, "(split-half|split half|splithalf|spearman-brown)"),
    exclude_r2 = str_detect(substring_between_cronbach_and_estimate, "r2"),
    exclude_theta = str_detect(all, "(theta|θ)"),
    exclude_rho = str_detect(all, "(rho|ρ)"),
    exclude_icc = str_detect(all, "icc"),
    exclude_raa = str_detect(all, "raa"),
    exclude_specific_exclusions = str_detect(all, "based on internal consistency reliability values of .70, .80, and .90.")
    ) |>
  mutate(
    ...
    exclude_master = ifelse(
      ...
      exclude_comparison_present == TRUE |
        ...
        exclude_odd_string_present_2 == TRUE |
        exclude_kappa == TRUE |
        exclude_splithalf == TRUE |
        exclude_r2 == TRUE |
        exclude_theta == TRUE |
        exclude_rho == TRUE |
        exclude_s == TRUE |
        exclude_icc == TRUE |
        exclude_raa == TRUE |
        exclude_specific_exclusions == TRUE,
      TRUE, 
      FALSE
    ),
...
write_csv(data_processed, "../../data/processed/psychology/data_processed_psychology.csv")
