...
non_cron <- c(
  ...
  "spearman-?brown", # spearman-brown prediction coefficient
...
```{r}
str_sub("the internal consistency reliability estimate (alpha) was .92.", 53)
```


```{r}
data_extracted_consistency_or_reliability_and_alpha <- records_wide_modified %>% 
  # filter(DOI == "10.1037/t00030-000") %>% 
  select(doi = DOI, text = Reliability) |>
  # first find reference to internal consistency
  rowwise() |>
  mutate(locations = list(
  ...
  ungroup() |>
  unnest(locations) |>
  rename(first_subst_location_start = start, 
         first_subst_location_end = end) |>
  mutate(pre_ic  = str_sub(text,
                           start = first_subst_location_end - pre_window_size,
                           end   = first_subst_location_end - 1),
         different_stat_mention = first_subst_location_end + str_locate(str_sub(text,
                                                                    start = first_subst_location_end), non_crons)[,"start"],
         post_ic = str_sub(text, start = first_subst_location_end,
                           end = coalesce(
                             different_stat_mention,
                             str_length(text)))) |>  
  # then find reference to alpha within the result
  ...
  rename(location_start_alpha = start, 
         location_end_alpha = end) |>
  mutate(
    location_start = first_subst_location_end + location_end_alpha - 10,
    location_end = first_subst_location_end + location_end_alpha,
    pre  = str_sub(text,
...
d2 <- data_extracted_internal_consistency %>% 
  filter(doi == "10.1037/t00030-000")
```
