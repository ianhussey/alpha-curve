...
```{r}
"([a-zA-Z]|[\\(,\\),0,=+]| )\\.[0-9]{2,} \\(([a-zA-Z]|[\\(,\\),0,=+]| )\\.[0-9]{2,}, "

"([a-zA-Z]|[\\(,\\),0,=+]| )\\.[0-9]{2,} \\(" # lower bound of a CI
...
records_wide_modified <- records_wide %>% 
  mutate(
    ...
    reliability_mentions_alpha = str_detect(string = Reliability, pattern = "(cronbach('|’)?s? (alpha|α|a)|(coefficient) (alpha|α|a =|a=))"),
    ...
    reliability_mentions_omega = str_detect(string = Reliability, pattern = "omega|ω"),
...
non_cron <- c(
  ...
  "omega|ω", 
  "(\\bκ\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\bκ\\s*[=<>≥≤]|\\bkappa\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\bkappa\\s*[=<>≥≤])", # κ/kappa 
  "(\\bρ\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\bρ\\s*[=<>≥≤]|\\broh\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\broh\\s*[=<>≥≤])",  # ρ/roh
  "(\\bp\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\bp\\s*[=<>≥≤])", # p-value,
  "(\\br\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\br\\s*[=<>≥≤]|\\b𝑟\\s*\\(\\d+\\)\\s*[=<>≥≤]|\\b𝑟\\s*[=<>≥≤])", # r/𝑟
...
str_detect("intercorrelated", non_crons)
...
data_extracted_consistency_or_reliability_and_alpha <- records_wide_modified %>% 
  ...
  mutate(
    ...
    location_end = location_end + location_end_alpha,
    pre  = str_sub(text,
...
data_estimates <- data_extracted  %>% 
  ...
  mutate(
    ...
    exclude_comparison_present = str_detect(substring_between_cronbach_and_estimate, "([<>≥≤±≈]|\\bover\\b|larger|\\bexceed\\b|above|upper|greater|higher|\\bmore\\b|lower|below|bellow|smaller|\\bless\\b|at least|between|the order of|increase|decrease)"),
    ...
    exclude_comparison_present_before_estimate = str_detect(pre, "(bound(s)|above|higher|more|greater|below|less|lower)"),
    exclude_comparison_present_after_estimate = str_detect(post, "(bound(s)|or above|or higher|or more|or greater|or below|or less|or lower)"),
...
# data_processed %>% group_by(doi, alpha_string) %>% filter(n()>1) %>% arrange(doi, alpha_string) %>% select(doi,regex_label , location_alpha_start, alpha_string, post, all) %>% left_join(records_wide %>% select(doi = DOI, Reliability)) %>% mutate(alpha2 = str_sub(Reliability, location_alpha_start)) %>%  View()
...
`exclude_r_present` might be too draconian:
```{r}
# data_processed %>% 
#   filter(exclude_quant_comparison_present == FALSE &
#         exclude_comparison_present == FALSE &
#         exclude_comparison_present_immediately_after_estimate == FALSE &
#         exclude_comparison_present_immediately_before_estimate == FALSE &
#         exclude_comparison_present_before_estimate == FALSE &
#         exclude_comparison_present_after_estimate == FALSE &
#         exclude_cutoff_present == FALSE &
#         exclude_range_present == FALSE &
#         exclude_nonalphas_present == FALSE &
#         exclude_non_cron_mention_after_estimate == FALSE &
#         exclude_p_present == FALSE &
#         exclude_r_present == TRUE &
#         exclude_significance_present == FALSE &
#         exclude_doi_string_present == FALSE &
#         exclude_psychometrika_present == FALSE &
#         exclude_odd_string_present == FALSE &
#         exclude_kappa == FALSE &
#         exclude_s == FALSE) %>% 
#   select(doi, alpha_string, substring_between_cronbach_and_estimate, all) %>% 
#   View
```
...
browseURL("https://youtu.be/U_ugnhUFBE8?t=16")
...
lots of occurrences of two main problem that i now fixed. one issue remains in the second extraction: non_crons estimates being extracted despite the `different_stat_mention` variable.
